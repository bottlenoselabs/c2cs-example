...
function create_build_files() {
    rm -rf $CMAKE_PROJECT_BUILD_DIRECTORY_PATH # Clear any previous build files
    ...
    cmake -S "./" -B $CMAKE_PROJECT_BUILD_DIRECTORY_PATH $CMAKE_ARCH_ARGS \
        `# change output directories` \
        -DCMAKE_ARCHIVE_OUTPUT_DIRECTORY="$CMAKE_PROJECT_BIN_DIRECTORY" -DCMAKE_LIBRARY_OUTPUT_DIRECTORY="$CMAKE_PROJECT_BIN_DIRECTORY" -DCMAKE_RUNTIME_OUTPUT_DIRECTORY="$CMAKE_PROJECT_BIN_DIRECTORY" \
        `# project specific` \
        $CMAKE_ARGS
...
function build() {
    ...
    cmake --build $CMAKE_PROJECT_BUILD_DIRECTORY_PATH --config Release
    ...
    local LIBRARY_FILE_PATH_BUILD=`get_full_path $CMAKE_PROJECT_BIN_DIRECTORY/$TARGET_BUILD_LIBRARY_FILENAME_SEARCH`
    ...
    if [[ "$OS" == "windows" ]]; then
        local LIBRARY_FILE_PATH_BUILD_LIB=`get_full_path "$CMAKE_PROJECT_BIN_DIRECTORY/$TARGET_BUILD_LIBRARY_FILENAME_LIB_SEARCH"`
    ...
    local FULL_OUTPUT_DIRECTORY=`get_full_path $OUTPUT_DIRECTORY`
    ...
    if [[ "$TARGET_BUILD_OS" == "windows" ]]; then
        LIBRARY_FILE_PATH_LIB="$OUTPUT_DIRECTORY/$TARGET_BUILD_LIBRARY_FILENAME_LIB"
